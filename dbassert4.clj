...
(defn val-3
  ...
  (let [f (:x mp)
	...
	m-1 (-> month prev f)
	m-2 (-> month prev prev f)]
    (map (fn [a b c] (vector (if (and (or (nil? a) (= 0 a))
				      (or (nil? b) (= 0 b))
				      (or (nil? c) (= 0 c)))
			       1 0))))
    m m-1 m-2))
...
;;	   (rule. 8 "Manual Sev-2 MTRS"  val-1 {:pred nvl-dbl- :x ms-sev2 :y md-sev2})
	   ...
	   (rule. 12 "Sev-1 consistently null or zero" val-3 {:x as-sev1})
	   (rule. 13 "Sev-2 consistently null or zero" val-3 {:x as-sev2})
	   (rule. 14 "CSR consistently null or zero" val-3 {:x as-csr})
	   (rule. 15 "Unix images peak utilization consistently null or zero" val-3 {:x as-unixpu})
	   (rule. 16 "Intel images peak utilization consistently null or zero" val-3 {:x as-winpu})
	   (rule. 17 "Security remediations on time consistently null or zero" val-3 {:x as-srot})
	   (rule. 18 "SAS70 recommendations on time consistently null or zero" val-3 {:x as-sas70})))

(def months ["2010-10-01" "2010-11-01" "2010-12-01" "2011-01-01" "2011-02-01" "2011-03-01"])

(validate rules months)

(dotimes [i 1] (future (validate rules (vector (nth months i)))))
