...
(def *db* {:classname "com.ibm.db2.jcc.DB2Driver"
	   :subprotocol "db2"
	   :subname "//b03cxnp46023.gho.boulder.ibm:60001/cqm_prod:retrieveMessagesFromServerOnGetMessage=true;"
	   :user "tashchep"
	   :password "rain2snow"})
...
(defn rule-maker-significant-diff
  [id name f]
  "Combines id, name and data retrieval function f into a
   rule 'Significant difference in number of tickets'"
  (letfn [(noz? [x] (or (nil? x) (zero? x)))]
    (rule. id name
	   (fn [now past]
	     (cond (nil? past) false
...
(def rules
     (list (rule. 1 "Sev-1 summary/details mismatch" #(not (= %1 %2)) [as-sev1 ad-sev1])
	   ...
	   (rule-maker-consistent-noz 20 "Incidents related to capacity consistently null or zero" as-ccri)
	   (rule-maker-significant-diff 21 "Significant difference in Sev-1 tickets [now past]" cd-sev1)
	   (rule-maker-significant-diff 22 "Significant difference in Sev-2 tickets [now past]" cd-sev2)
	   (rule-maker-significant-diff 23 "Significant difference in change tickets [now past]" cd-csr)
	   ))
...
;; for interactrive development use this call:
;; (time (main rules ["2011-05-01"]))

;; Using pmap instead of map decreases execution time:
;;  - sequential rule materialization takes 71 sec
;;  - parallel only 51 sec

(main rules *command-line-args*)
