...
(defn validate
  ...
  (doseq [month months]
    ...
    (doseq [rule rules]
      (println (format "\n Rule %d: %s\n" (:id rule) (:title rule)))
      (pretty-print (vec (map (:pred rule) ((:x rule) month) ((:y rule) month)))))))


(load-queries *queries*)

(def rules
     (list (rule. 1 "Auto Sev-1 MTRS"    nvl-dbl- as-sev1 ad-sev1)
	   (rule. 2 "Auto Sev-2 MTRS"    nvl-dbl- as-sev2 ad-sev2)
	   (rule. 3 "Auto Sev-1 tickets" nvl-int- ca-sev1 cd-sev1)
	   (rule. 4 "Auto Sev-2 tickets" nvl-int- ca-sev2 cd-sev2)
	   (rule. 5 "Auto CSR metric"    nvl-dbl- as-csr  ad-csr)
	   (rule. 6 "Auto CSR tickets"   nvl-int- cs-csr  cd-csr)
	   (rule. 7 "Manual Sev-1 MTRS"  nvl-dbl- ms-sev1 md-sev1)
	   (rule. 8 "Manual Sev-2 MTRS"  nvl-dbl- ms-sev2 md-sev2)))
