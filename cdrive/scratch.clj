...
(defn add-plants
  "Adds two random plants, one of them in a jungle"
   [plants]
...
(defn turn
  ...
  [anim]
  (let [x (random (apply + (:genes anim)))]
    (struct animal
	    (:x anim)
	    (:y anim)
	    (:energy anim)
	    (mod (+ (:dir anim) (angle (:genes anim) x)) 8)
	    (:genes anim))))
...
and updated plants (second)"
  ...
  (let [pos (list (:x anim) (:y anim))]
    (if (contains? plants pos)
      (list (struct animal (:x anim) (:y anim) (+ (:energy anim) *plant-energy*)
	            (:dir anim) (:genes anim))
	    (disj plants pos))
      (list (struct animal (:x anim) (:y anim) (:energy anim)
		    (:dir anim) (:genes anim)) plants))))
...
(defn reproduce [anim]
  "Reproduces itself. Returns a list of new animals, including self"
  (let [e (:energy anim)]
    (if (>= e *reproduction-energy*)
      (let [nue (bit-shift-right e 1)]
	(list (struct animal (:x anim) (:y anim) nue (:dir anim) (:genes anim))
	      (struct animal (:x anim) (:y anim) nue (:dir anim) (mutate (:genes anim)))))
      (list anim))))
...
(defn world-turns
  "Simulates one day in a world. Returns updates animals (first), and updated plants (second)"
  ([world]
     (world-turns (first world) (second world)))
  ([animals plants]
...
(defn step
  ...
  ([times]
     (dotimes [i times]
       (let [w1 (world-turns *animals* *plants*)]
...
(defn stats
  ...
  (print (format "|\n %s animals, %s plants\n"
		 (count animals)
		 (count plants)))
  (print "Five strongest animals:")
  (println (take 5 (sort-by #(* -1 (:energy %)) animals)))
